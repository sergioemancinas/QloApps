<div id="opc_new_account" class="opc-main-block">
	...
	{block name='order_opc_new_account_choice'}
		<div id="login_form">
			<fieldset>
				<div class="opc-choice row">
					<div class="col-sm-6">
	...
	{block name='order_opc_new_account_new_account_form'}
		<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="new_account_form" class="std" autocomplete="on" autofill="on">
			...
			<div id="opc_account_form" class="unvisible">
				...
				<p class="text-muted">{l s='Guest checkout — details are used only for this booking.'}</p>
				...
				<div class="row">
					...
					<div class="required form-group col-sm-6">
						...
						<input type="text" class="text form-control validate" id="customer_lastname" name="customer_lastname" autocomplete="family-name" onblur="$('#lastname').val($(this).val());" data-validate="isName" value="{if isset($guestInformations) && isset($guestInformations.customer_lastname) && $guestInformations.customer_lastname}{$guestInformations.customer_lastname}{/if}" />
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}form-group col-sm-6">
						...
						<input type="text" class="text form-control validate" name="customer_phone" id="customer_phone" autocomplete="tel" data-validate="isPhoneNumber" value="{if isset($guestInformations) && isset($guestInformations.phone_mobile) && $guestInformations.phone_mobile}{$guestInformations.phone_mobile}{/if}" />
					</div>
				</div>
					{if true}
					{* Guest checkout keeps the form minimal: no date of birth,
					   newsletter, partner offers, password or duplicate contact fields.
					   Accounts (with those extras) are handled by Keycloak registration. *}
					<p class="block-small-header margin-top-20 margin-btm-10">{l s='BOOKING ADDRESS'}</p>
					{$stateExist = false}
					{$postCodeExist = false}
					{$dniExist = false}
					<div class="row">
						{foreach from=$dlv_all_fields item=field_name}
							...
							{elseif $field_name eq "company"}
								{* Company optional — hidden for a minimal guest form *}
						...
						{/if}
						{if !$dniExist}
							<div class="required dni form-group col-sm-6" style="display:none;">
								<label for="dni">{l s='Identification number'}</label>
								<input type="text" class="text form-control validate" name="dni" id="dni" data-validate="isDniLite" value="{if isset($guestInformations) && isset($guestInformations.dni) && $guestInformations.dni}{$guestInformations.dni}{/if}" />
							</div>
						{/if}
						<div class="form-group is_customer_param col-sm-6" style="display:none;">
